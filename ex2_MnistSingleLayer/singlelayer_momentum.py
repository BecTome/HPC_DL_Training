...
plt.legend()
plt.title(f"Momentum Optimizer with different LR (Mom={MOMENTUM}))")
plt.xlabel("Epochs")
plt.ylabel("Loss")
filename = f"single_layer_momentum_{N.min(ls_lr)}-{N.max(ls_lr)}_momentum_{MOMENTUM}".replace(".", "_") + ".pdf"
